	...
	<jsp:include page="../navbar.jsp" />
     ...
     points.push(marker) */
		...
		<script
			...
			<div class="card" style="height:fit-content;width: 20rem;background-color: #F0FFFF;margin-top:59px;">
			<div class="card-body">
				<h4 class="card-title" style="font-weight:400;">${point.name }</h4>
				<p class="card-text" style="font-weight:400;">${point.address.getCity() }</p>
			</div>
			<div>
				<c:forEach var="amenity" items="${point.getAmenities()}">
					<a href="viewAmenity.do?id=${amenity.id }" style="margin-left:20px;font-weight:400;">
					<span>${amenity.iconUrl }</span></a>
				</c:forEach>
			</div>
				<p class="card-text" style="margin-left:20px; font-weight:400;">${point.address.getStreet1() }</p>
				<p class="card-text" style="margin-left:20px; font-weight:400;">${point.address.getCity() }  ${point.address.getZipcode() }</p>
				<p class="card-text" style="margin-left:20px; font-weight:400;">${point.address.getPhone() }</p>
				<p class="card-text" style="margin-left:20px; font-weight:400;">latitude :${point.address.latitude },
			...
			<div>
				...
				<h3>
					...
					data-target="#addCommentModal" style="margin-left:20px;color:deepskyblue;font-weight:400;"><span><i class="fas fa-comments"></i></span></a>
				</h3>
				<c:choose>
					<c:when test="${! empty sessionScope.user}">
						<a href="#review" class="navbar-brand" rel="tooltip"
							title="login" data-placement="bottom" data-toggle="modal"
							data-target="#tripModal" style="color:deepskyblue;margin-left:20px;font-weight:400"> Add To Trip Journal </a>
						<br>
					</c:when>
				</c:choose>
			</div>
